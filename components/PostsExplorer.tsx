    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-3 md:items-end">
        <div className="space-y-2 md:col-span-2">
          <PixelLabel>{dict.postsExplorer.searchLabel}</PixelLabel>
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder={dict.postsExplorer.searchPlaceholder}
        ...
        <div className="space-y-2">
          <PixelLabel>{dict.postsExplorer.sortLabel}</PixelLabel>
          ...
          <div className="relative">
            ...
            >
              <option value="newest">{dict.postsExplorer.sortNewest}</option>
              <option value="oldest">{dict.postsExplorer.sortOldest}</option>
            </select>
            <span
              aria-hidden
      ...
      <div className="space-y-2">
        <div className="flex items-end justify-between gap-3">
          <PixelLabel>{dict.postsExplorer.tagsLabel}</PixelLabel>
      ...
      {results.length === 0 ? (
        <p className="text-sm text-[rgba(11,15,20,0.65)]">{dict.postsExplorer.empty}</p>
      ) : null}
    </div>
  )
}
